...
<div>
    <form autocomplete="off" action="{{ route('search2') }}" method="get" class="flex items-center">
        @csrf
            <div class="{{ $attributes }} relative">
                <label for="search" class="sr-only">Name</label>
                <input id="{{ $name }}" type="text" name="search" placeholder="Vyhľadaj" class="bg-gray-100 border-2 border-gray-100 w-full p-1 rounded-lg mr-2 focus:outline-none focus:border-gray-400 focus:ring-0 hover:border-gray-300 trasition duration-500">
            </div>
...
<script>
    autocomplete(document.getElementById("{{$name}}"), @json($values));
</script>