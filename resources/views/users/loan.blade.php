...
@section('content')
    <div class="flex justify-center w-full ">
        <div class="w-8/12 bg-white ml-6 mt-6 p-6 rounded-l-lg" >
            @if ($user->loan)
                <h1 class="font-bold text-3xl mb-2">Vaša výpožička</h1>
                <div class="mb-4 ml-4"> 
                    <p>Vypozicanych knih: {{ $user->loan->books->count() }}</p>
                    @if ($user->loan->approved == 1)   
                        <p>Vypožičané od: {{ $user->loan->from }}</p>
                        <p>Vypožičané do: {{ $user->loan->to }}</p>
                    @endif
                </div>
                <h1 class="font-bold text-3xl mb-2">Vypožičané knihy</h1>
