        ...
        <div class="w-full mt-6 ml-6 bg-white p-6 rounded-tl-lg" >
            <form method="POST" action="{{ route('school.update', $school) }}">
                ...
                <div class="mb-4">
                    ...
                    <input class="bg-gray-100 border-2 w-full p-4 text-gray-600 rounded-lg focus:outline-none focus:border-gray-400 focus:ring-0 hover:border-gray-300 @error('name') border-red-500 @enderror trasition duration-500" type="text" id="name" name="name" value="{{ $school->name }}">
                </div>
                @error('name')
